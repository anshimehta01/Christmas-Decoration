...
window.addEventListener("click", e => {
    ...
    if (intersects) {
        addBall(intersects[0].point, balls.length - 2, performance.now());
    }
});
...
function initScene() {
   ...
   camera.lookAt(0, 0, 0);
    ...
    sideLight.shadow.mapSize.width = 2030;
    sideLight.shadow.mapSize.height = 2030;
...
function render(time) {
    ...
    balls.forEach((b, bIdx) => {
        const aT = .0012 * (time - b.clickTime);
